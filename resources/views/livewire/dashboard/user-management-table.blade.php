...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 opacity-0 animate-[fadeUp_0.5s_ease_0.1s_forwards]">
    @foreach([
        ['label'=>'Total Users','value'=>'5','icon'=>'group','color'=>'bg-[#012d1d] text-white','shadow'=>'shadow-[4px_4px_0_0_#012d1d]'],
        ['label'=>'Admin & Employee','value'=>'2','icon'=>'badge','color'=>'bg-[#d3ee6f] text-[#212a00]','shadow'=>'shadow-[4px_4px_0_0_#012d1d]'],
        ['label'=>'Customer','value'=>'2','icon'=>'person','color'=>'bg-[#dde4e0] text-[#012d1d]','shadow'=>'shadow-[4px_4px_0_0_#012d1d]'],
        ['label'=>'Inactive Accounts','value'=>'1','icon'=>'block','color'=>'bg-[#ffdad6] text-[#ba1a1a]','shadow'=>'shadow-[4px_4px_0_0_#ba1a1a] border-[#ba1a1a]'],
    ] as $stat)
...
<section class="border-[3px] border-[#012d1d] bg-white shadow-[4px_4px_0_0_#012d1d] overflow-hidden opacity-0 animate-[fadeUp_0.5s_ease_0.2s_forwards]">
    <div class="px-6 py-4 border-b-[3px] border-[#012d1d] bg-[#dde4e0] flex items-center justify-between">
        ...
        <span class="font-label font-bold text-xs text-[#012d1d] bg-[#d3ee6f] px-3 py-1 border-[2px] border-[#012d1d]">5 accounts</span>
    </div>
            ...
            <tr class="row-anim hover:bg-[#f4fbf7] transition-colors group relative" wire:key="user-{{ $user->id }}">
                ...
                <td class="px-6 py-4">
                    <div class="flex items-center justify-start gap-1.5">
                        <button @click="showEdit = true" title="Edit"
...
<div x-show="showCreate" x-cloak @keydown.escape.window="showCreate=false"
    ...
    <div class="border-[3px] border-[#012d1d] bg-white w-full max-w-md shadow-[8px_8px_0_0_#012d1d]"
        ...
        <div class="p-8 flex flex-col gap-5">
            @foreach([['Full Name','text','John Doe',true],['Email','email','john@dapute.com',true],['Phone','text','+62 8123456789',false],['Initial Password','password','Min. 8 characters',true]] as [$lbl,$typ,$ph,$req])
            <div>
                <label class="block font-label font-bold text-[10px] uppercase tracking-[0.2em] text-[#012d1d] mb-2">{{ $lbl }}@if($req)<span class="text-[#ba1a1a]"> *</span>@endif</label>
                <input type="{{ $typ }}" placeholder="{{ $ph }}"
        ...
        <div class="px-8 pb-8 flex gap-3">
            ...
            <button class="flex-1 py-3 border-[3px] border-[#012d1d] bg-[#012d1d] text-white font-label font-bold text-xs uppercase tracking-wider hover:bg-[#1b4332] transition-all shadow-[2px_2px_0_0_#012d1d] flex items-center justify-center gap-2">
...
{{-- ══ MODAL: EDIT ════════════════════════════════════════════════ --}}
<div x-show="showEdit" x-cloak @keydown.escape.window="showEdit=false"
     x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
     x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
     class="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm" style="background:rgba(55,65,81,0.60);">
    <div class="border-[3px] border-[#012d1d] bg-white w-full max-w-md shadow-[8px_8px_0_0_#012d1d]"
        ...
        <div class="p-8 flex flex-col gap-5">
            <div>
                ...
                <input type="text" value="Budi Santoso" class="w-full bg-[#eef5f1] border-[3px] border-[#012d1d] px-4 py-3 font-body text-sm text-[#012d1d] focus:outline-none focus:bg-white focus:shadow-[4px_4px_0_0_#012d1d] transition-all">
            </div>
            <div>
                ...
                <input type="email" value="budi@dapute.com" class="w-full bg-[#eef5f1] border-[3px] border-[#012d1d] px-4 py-3 font-body text-sm text-[#012d1d] focus:outline-none focus:bg-white focus:shadow-[4px_4px_0_0_#012d1d] transition-all">
            </div>
            <div>
                <label class="block font-label font-bold text-[10px] uppercase tracking-[0.2em] text-[#012d1d] mb-2">Phone Number</label>
                <input type="text" value="08123456789" class="w-full bg-[#eef5f1] border-[3px] border-[#012d1d] px-4 py-3 font-body text-sm text-[#012d1d] focus:outline-none focus:bg-white focus:shadow-[4px_4px_0_0_#012d1d] transition-all">
            </div>
            <div>
                ...
                <div class="relative">
                    <select class="w-full appearance-none bg-[#eef5f1] border-[3px] border-[#012d1d] px-4 py-3 pr-10 font-label font-bold text-sm text-[#012d1d] focus:outline-none focus:bg-white focus:shadow-[4px_4px_0_0_#012d1d] transition-all cursor-pointer">
            ...
            <div class="flex items-center justify-between border-[3px] border-[#012d1d] bg-[#eef5f1] px-4 py-4">
                ...
                <div x-data="{on:true}">
                    <button type="button" @click="on=!on"
                        ...
                        :style="on ? 'background:#d3ee6f' : 'background:#dde4e0'">
                        <span class="absolute top-[2px] w-5 h-5 border-[3px] border-[#012d1d] transition-all pointer-events-none"
                            :style="on ? 'left:calc(100% - 1.375rem); background:#012d1d' : 'left:1px; background:#717973'"></span>
                    </button>
                </div>
            </div>
        </div>
